    ...
    public String establishConference(String hostExtension) {
        ...
        try {
            ...
            try {
                ...
                eventData.put("participants", String.join(",", session.participants));
                eventData.put("timestamp", String.valueOf(System.currentTimeMillis()));
                sseService.sendEvent(hostExtension, "phone_event", eventData);
                
                // *** 新增：通知所有參與者 ***
                for (String participant : session.participants) {
                    if (!participant.equals(hostExtension)) {
                        Map<String, String> participantEventData = new HashMap<>();
                        participantEventData.put("action", "joined_conference");
                        participantEventData.put("host", hostExtension);
                        participantEventData.put("timestamp", String.valueOf(System.currentTimeMillis()));
                        sseService.sendEvent(participant, "phone_event", participantEventData);
                    }
                }
