...
public class PhoneCallService {
    ...
    // 用於追蹤已設置監聽器的分機
    private final Set<String> extensionsWithListeners = ConcurrentHashMap.newKeySet();
    
    /**
     * 為指定分機建立JTAPI事件監聽器
     */
    public void setupListenerForExtension(String extension) {
        // 檢查是否已經設置過監聽器
        if (extensionsWithListeners.contains(extension)) {
            System.out.println("[SETUP_LISTENERS] 分機 " + extension + " 已經設置過監聽器，跳過");
            return;
        }
        
        ExtensionConnection ctiConn = findCTIConnection();
        if (ctiConn == null) {
            System.err.println("[SETUP_LISTENERS] 沒有可用的CTI連接，無法為分機 " + extension + " 設置監聽器");
            return;
        }
        
        try {
            System.out.println("[SETUP_LISTENERS] 開始為分機 " + extension + " 建立監聽器...");
            ...
            if (terminals != null && terminals.length > 0) {
                ...
                extensionsWithListeners.add(extension); // 記錄已設置
                System.out.println("[SETUP_LISTENERS] ✅ 已為分機 " + extension + " 建立監聽器");
            } else {
                System.out.println("[SETUP_LISTENERS] ❌ 分機 " + extension + " 沒有可用的Terminal");
            }
        } catch (Exception e) {
            System.err.println("[SETUP_LISTENERS] ❌ 為分機 " + extension + " 建立監聽器失敗: " + e.getMessage());
        }
    }
    ...
    private void sendUpdateEvent(String extension, String eventType, String message) {
        try {
            ...
            System.out.println("[SSE_UPDATE] 已推送 " + eventType + " 事件到分機 " + extension);
        } catch (Exception e) {
            System.err.println("[SSE_UPDATE] 推送事件失敗: " + e.getMessage());
        }
    }
            ...
            CallObserver callObserver = new CallObserver() {
                ...
                public void callChangedEvent(CallEv[] eventList) {
                    ...
                    for (CallEv event : eventList) {
                        ...
                        // *** 話機直接操作SSE推送 ***: 無論什麼事件都推送狀態更新
                        // 這確保話機直接操作時前端能立即收到更新
                        try {
                            Map<String, String> phoneEventData = new HashMap<>();
                            phoneEventData.put("action", "phone_state_change");
                            phoneEventData.put("extension", extension);
                            phoneEventData.put("eventType", eventClassName);
                            phoneEventData.put("message", "話機狀態變化");
                            phoneEventData.put("timestamp", String.valueOf(System.currentTimeMillis()));
                            ...
                            try {
                                ...
                                // 觸發狀態同步，確保話機操作立即反映到統一話機服務
                                unifiedPhoneService.syncPhoneStateFromJTAPI(extension);
                                System.out.println("[PHONE_DIRECT_SSE] 已觸發UnifiedPhoneService狀態同步");
                            } catch (Exception syncError) {
                                System.err.println("[PHONE_DIRECT_SSE] 觸發狀態同步失敗: " + syncError.getMessage());
                            }
                        } catch (Exception sseError) {
                            System.err.println("[PHONE_DIRECT_SSE] 推送事件失敗: " + sseError.getMessage());
                        }
                    }
                }
            };
